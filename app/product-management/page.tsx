...
import {
  ...
  updateArticleCode,
  deleteArticleCode,
  uploadArticleCodesCsv,
  updateArticleCodesCsv,
...
import {
  ...
  uploadPriceConsolidatedCsv,
  updatePriceConsolidatedCsv,
...
function StoreProductsTab() {
  ...
  const handleCsvUpload = async () => {
    ...
    try {
      ...
      setError(null);
      const result = await uploadStoreProductsCsv(csvFile, csvUpsert);
      setCsvResult(result);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              {!csvResult ? (
                <>
                  <div className="mb-4">
                    ...
                    <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                      CSV must contain columns: ykey, product_name, store, state
                    </p>
                  </div>
                  <div className="mb-4">
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        checked={csvUpsert}
                        onChange={(e) => setCsvUpsert(e.target.checked)}
                      ...
                      <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                        Update existing entries (upsert mode)
                      </span>
                    </label>
                  </div>
                  <div className="flex justify-end gap-3">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-400 text-white rounded-lg font-medium"
                    >
                ...
                <>
                  <div className="mb-4">
                    <div className={`p-4 rounded-lg ${csvResult.success ? 'bg-green-50 dark:bg-green-900/20' : 'bg-yellow-50 dark:bg-yellow-900/20'}`}>
                      <h3 className={`font-semibold mb-2 ${csvResult.success ? 'text-green-800 dark:text-green-400' : 'text-yellow-800 dark:text-yellow-400'}`}>
                        {csvResult.success ? 'Upload Successful' : 'Upload Completed with Errors'}
                      </h3>
                      <div className="text-sm space-y-1">
                        <p className="text-gray-700 dark:text-gray-300">Total rows: {csvResult.total_rows}</p>
                        <p className="text-green-600 dark:text-green-400">Created: {csvResult.created_count}</p>
                        <p className="text-blue-600 dark:text-blue-400">Updated: {csvResult.updated_count}</p>
                        <p className="text-gray-600 dark:text-gray-400">Skipped: {csvResult.skipped_count}</p>
                        <p className="text-red-600 dark:text-red-400">Failed: {csvResult.failed_count}</p>
                        <p className="text-gray-600 dark:text-gray-400">Processing time: {csvResult.processing_time_seconds.toFixed(2)}s</p>
                      </div>
                    </div>
                  </div>
                  {csvResult.errors.length > 0 && (
                    <div className="mb-4 max-h-64 overflow-y-auto">
                      <h4 className="font-semibold text-red-800 dark:text-red-400 mb-2">Errors:</h4>
                  ...
                  {csvResult.warnings.length > 0 && (
                    <div className="mb-4">
                      <h4 className="font-semibold text-yellow-800 dark:text-yellow-400 mb-2">Warnings:</h4>
                      <ul className="list-disc list-inside text-sm text-yellow-700 dark:text-yellow-300">
                        {csvResult.warnings.map((warning, idx) => (
...
function ArticleCodesTab() {
  ...
  const [csvFile, setCsvFile] = useState<File | null>(null);
  const [csvUploading, setCsvUploading] = useState(false);
  const [csvCreateResult, setCsvCreateResult] = useState<{
    success: boolean;
    total_rows: number;
    created_count: number;
    updated_count: number;
  ...
  const [csvUpdateResult, setCsvUpdateResult] = useState<{
    success: boolean;
    total_rows: number;
    updated_count: number;
    skipped_count: number;
  ...
  const handleBulkCreateCsv = async () => {
    if (!csvFile) {
    ...
    try {
      setCsvUploading(true);
      setError(null);
      const result = await uploadArticleCodesCsv(csvFile);
      setCsvCreateResult(result);
      ...
      setError(err instanceof Error ? err.message : 'Failed to upload CSV');
      setCsvCreateResult(null);
    } finally {
      setCsvUploading(false);
    }
  };

  const handleBulkUpdateCsv = async () => {
    if (!csvFile) {
      setError('Please select a CSV file');
      return;
    }
    ...
    try {
      setCsvUploading(true);
      setError(null);
      const result = await updateArticleCodesCsv(csvFile);
      setCsvUpdateResult(result);
...
function PricePosTab() {
  ...
  // State for managing multiple Point of Sale entries
  const [posEntries, setPosEntries] = useState<string[]>([]);
  ...
  const handleAddPosToList = (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentPos.trim()) return;
    // Add current Point of Sale to list
    setPosEntries([...posEntries, currentPos.trim()]);
    // Reset POS field
    setCurrentPos('');
  };

  const handleRemovePosEntry = (index: number) => {
    setPosEntries(posEntries.filter((_, i) => i !== index));
  };

  const handleCreateAllPricePos = async () => {
    console.log('handleCreateAllPricePos called');
    console.log('posEntries:', posEntries);
    console.log('formData:', formData);

    if (posEntries.length === 0) {
      setError('Please add at least one Point of Sale before submitting');
      alert('Please add at least one Point of Sale before submitting');
      return;
    }

    if (!formData.state || !formData.promoter || !formData.pricelist) {
      setError('Please fill in State, Promoter, and Pricelist');
      alert('Please fill in State, Promoter, and Pricelist');
      return;
    }

    try {
      // Create an entry for each Point of Sale with the same State, Promoter, and Pricelist
      const entries = posEntries.map(pos => ({
      ...
      const result = await bulkCreatePricePos({ entries });
      ...
      const successMsg = `Success: Created ${result.created_count}, Updated ${result.updated_count}`;
      const failMsg = result.failed_count > 0 ? `, Failed ${result.failed_count}` : '';
      ...
      console.error('Error in handleCreateAllPricePos:', err);
      const errorMessage = err instanceof Error ? err.message : 'Failed to create price POS mappings';
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Create Price POS Mappings</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Enter common details and add multiple Point of Sale locations
              </p>
            </div>
            <div className="p-6">
              {/* Common fields - State, Promoter, Pricelist */}
              <div className="mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Common Details</h3>
              ...
              {/* Add Point of Sale */}
              <div className="mb-6">
                <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Point of Sale Locations</h3>
                <form onSubmit={handleAddPosToList} className="flex gap-3">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium flex items-center gap-2 whitespace-nowrap"
                  >
                    <span>+</span> Add
                  </button>
                </form>
              ...
              {/* List of Point of Sale entries */}
              {posEntries.length > 0 && (
                <div className="mb-6">
                  <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                    Added Locations ({posEntries.length})
                  </h4>
                      ...
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0">
                          <tr>
                            ...
                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Point of Sale</th>
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                          {posEntries.map((pos, index) => (
                            <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                              ...
                              <td className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400">{pos}</td>
                              <td className="px-4 py-2 text-sm">
                                <button
                                  type="button"
                                  onClick={() => handleRemovePosEntry(index)}
                                  className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300"
                                >
                                  Remove
                                </button>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button
                  type="button"
                  onClick={handleCreateAllPricePos}
                  disabled={posEntries.length === 0}
...
function PriceConsolidatedTab() {
  ...
  // CSV Upload States
  const [isBulkCreateCsvModalOpen, setIsBulkCreateCsvModalOpen] = useState(false);
  const [isBulkUpdateCsvModalOpen, setIsBulkUpdateCsvModalOpen] = useState(false);
  const [csvFile, setCsvFile] = useState<File | null>(null);
  const [csvUploading, setCsvUploading] = useState(false);
  const [csvCreateResult, setCsvCreateResult] = useState<{
    success: boolean;
  ...
  const handleBulkCreateCsv = async () => {
    ...
    try {
      setCsvUploading(true);
      setError(null);
      const result = await uploadPriceConsolidatedCsv(csvFile);
      setCsvCreateResult(result);
  ...
  const handleBulkUpdateCsv = async () => {
    ...
    try {
      setCsvUploading(true);
      setError(null);
      const result = await updatePriceConsolidatedCsv(csvFile);
      setCsvUpdateResult(result);
  ...
  const resetBulkCreateCsvModal = () => {
    setIsBulkCreateCsvModalOpen(false);
    setCsvFile(null);
    setCsvCreateResult(null);
    setError(null);
  };

  const resetBulkUpdateCsvModal = () => {
    setIsBulkUpdateCsvModalOpen(false);
    setCsvFile(null);
    setCsvUpdateResult(null);
    setError(null);
  };
    ...
    <div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          ...
          <button
            onClick={() => setIsBulkCreateCsvModalOpen(true)}
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors"
          >
            Bulk Create (CSV)
          </button>
          <button
            onClick={() => setIsBulkUpdateCsvModalOpen(true)}
            className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-medium transition-colors"
          >
            Bulk Update (CSV)
          </button>
...
Smart Bazaar, Cashew W320 With Skin, 825.00, 0.05"
      ...
      {/* Bulk Create CSV Modal */}
      {isBulkCreateCsvModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Bulk Create Price Consolidated (CSV)</h2>
            </div>
            <div className="p-6">
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                  Example: Smart Bazaar,Almonds Non Pareil Running (25-29) Loose FG,1250.50,0.05
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Note: GST column is optional. If (pricelist, product) exists, it will be updated.
                </p>
              </div>
              ...
              {csvCreateResult && (
                <div className={`mb-4 p-4 rounded-lg ${csvCreateResult.success ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800'}`}>
                  <h3 className="font-semibold text-sm mb-2 text-gray-900 dark:text-white">Upload Results</h3>
                  <div className="text-sm space-y-1">
                    <p className="text-gray-700 dark:text-gray-300">Total Rows: {csvCreateResult.total_rows}</p>
                    <p className="text-green-600 dark:text-green-400">Created: {csvCreateResult.created_count}</p>
                    <p className="text-blue-600 dark:text-blue-400">Updated: {csvCreateResult.updated_count}</p>
                    <p className="text-gray-600 dark:text-gray-400">Skipped: {csvCreateResult.skipped_count}</p>
                    <p className="text-red-600 dark:text-red-400">Failed: {csvCreateResult.failed_count}</p>
                    <p className="text-gray-600 dark:text-gray-400">Processing Time: {csvCreateResult.processing_time_seconds.toFixed(2)}s</p>
                  </div>

                  {csvCreateResult.errors.length > 0 && (
                    <div className="mt-3">
                      <h4 className="font-semibold text-sm mb-2 text-red-600 dark:text-red-400">Errors:</h4>
                      <div className="max-h-40 overflow-y-auto space-y-2">
                        {csvCreateResult.errors.map((err, idx) => (
                          <div key={idx} className="text-xs bg-white dark:bg-gray-800 p-2 rounded border border-red-200 dark:border-red-800">
                            <p className="font-semibold text-red-600 dark:text-red-400">Row {err.row}: {err.error}</p>
                            <p className="text-gray-600 dark:text-gray-400 mt-1">Data: {JSON.stringify(err.data)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="flex justify-end gap-3">
                <button
                  type="button"
                  onClick={resetBulkCreateCsvModal}
                  className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700"
                  disabled={csvUploading}
                >
                  {csvCreateResult ? 'Close' : 'Cancel'}
                </button>
                {!csvCreateResult && (
                  <button
                    onClick={handleBulkCreateCsv}
                    disabled={!csvFile || csvUploading}
                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white rounded-lg font-medium"
                  >
                    {csvUploading ? 'Uploading...' : 'Upload & Create'}
                  </button>
                )}
              </div>
            </div>
      ...
      {isBulkUpdateCsvModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Bulk Update Price Consolidated (CSV)</h2>
            </div>
            <div className="p-6">
              <div className="mb-4">
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                  Matching Strategy: Matches by exact pricelist AND product. Updates price and/or GST.
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Example: Smart Bazaar,Almonds Product,1300.00,0.05 (leave price or gst empty to skip updating that field)
                </p>
              </div>
              ...
              {csvUpdateResult && (
                <div className={`mb-4 p-4 rounded-lg ${csvUpdateResult.success ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800'}`}>
                  <h3 className="font-semibold text-sm mb-2 text-gray-900 dark:text-white">Update Results</h3>
                  <div className="text-sm space-y-1">
                    <p className="text-gray-700 dark:text-gray-300">Total Rows: {csvUpdateResult.total_rows}</p>
                    <p className="text-blue-600 dark:text-blue-400">Updated: {csvUpdateResult.updated_count}</p>
                    <p className="text-gray-600 dark:text-gray-400">Skipped: {csvUpdateResult.skipped_count}</p>
                    <p className="text-red-600 dark:text-red-400">Failed: {csvUpdateResult.failed_count}</p>
                    <p className="text-orange-600 dark:text-orange-400">Not Found: {csvUpdateResult.not_found_count}</p>
                    <p className="text-green-600 dark:text-green-400">Matched by Pricelist & Product: {csvUpdateResult.matched_by_pricelist_product}</p>
                    <p className="text-gray-600 dark:text-gray-400">Processing Time: {csvUpdateResult.processing_time_seconds.toFixed(2)}s</p>
                  </div>

                  {csvUpdateResult.errors.length > 0 && (
                    <div className="mt-3">
                      <h4 className="font-semibold text-sm mb-2 text-red-600 dark:text-red-400">Errors:</h4>
                      <div className="max-h-40 overflow-y-auto space-y-2">
                        {csvUpdateResult.errors.map((err, idx) => (
                          <div key={idx} className="text-xs bg-white dark:bg-gray-800 p-2 rounded border border-red-200 dark:border-red-800">
                            <p className="font-semibold text-red-600 dark:text-red-400">Row {err.row}: {err.error}</p>
                            <p className="text-gray-600 dark:text-gray-400 mt-1">Data: {JSON.stringify(err.data)}</p>
                          </div>
              ...
              <div className="flex justify-end gap-3">
                <button
                  type="button"
                  onClick={resetBulkUpdateCsvModal}
                  className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700"
                  disabled={csvUploading}
                >
                  {csvUpdateResult ? 'Close' : 'Cancel'}
                </button>
                {!csvUpdateResult && (
                  <button
                    onClick={handleBulkUpdateCsv}
                    disabled={!csvFile || csvUploading}
